...
MarkHunter（马克猎手）是 Windows 桌面 Markdown/文本编辑器，Electron 43 + CodeMirror 6 + markdown-it + mermaid + Tailwind v4/daisyUI 5 主题引擎。中文注释与文档，无独立 lint/typecheck，靠 `npm run smoke`（真实窗口端到端，129 项）回归。
...
（空 —— 2026-08-22 审计的 5 项 + 验证期新发现的分段保存竞态已在 v0.1.46 全部修复，明细见 CHANGELOG；修复验证口径：`npm run smoke` 129 项全部通过，其中 5 项为信息型结果（`detectPython`/`runPython`/`findAll`/`pyUI`/`bundleSplit` 返回携带信息的真值串，非失败）。）
